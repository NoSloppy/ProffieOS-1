...
struct BlasterDisplayConfigFile : public ConfigFile {
  ...
  void iterateVariables(VariableOP *op) override {
    CONFIG_VARIABLE2(ProffieOSModeKillImageDuration, 1000.0f);
    CONFIG_VARIABLE2(ProffieOSModeAutoImageDuration, 1000.0f);
    ...
    switch (effect) {
      case EFFECT_MODE:
        switch (prop.GetBlasterMode()) {
          case Blaster::MODE_KILL:
            this->ShowFileWithSoundLength(&img_.IMG_mdkill, blaster_font_config.ProffieOSModeKillImageDuration);
            break;
          case Blaster::MODE_AUTO:
            this->ShowFileWithSoundLength(&img_.IMG_mdauto, blaster_font_config.ProffieOSModeAutoImageDuration);
            break;
          default:
            break;
        }
  ...
  void SB_Effect2(EffectType effect, EffectLocation location) override {
    switch (effect) {
      case EFFECT_MODE:
        switch (prop.GetBlasterMode()) {
          case Blaster::MODE_KILL:
            this->scr_.Play(&SCR_mdkill);
            break;
          case Blaster::MODE_AUTO:
            this->scr_.Play(&SCR_mdauto);
            break;
          default:
            break;
        }
